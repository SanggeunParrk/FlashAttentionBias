...
# ---------------------------------------------------------------------------
# Score-mod stubs — flash_bwd_sm90.py / flash_fwd.py still import these to
# satisfy `if const_expr(self.score_mod is not None): apply_score_mod_inner(...)`
# branches. Score-mod / mask-mod were stripped at the API level; these
# branches are const_expr-False and never execute, so the stubs raise if
# anyone reaches them.
# ---------------------------------------------------------------------------
def apply_score_mod_inner(*args, **kwargs):
    raise NotImplementedError("score_mod was stripped from this build.")


def apply_score_mod_bwd_inner(*args, **kwargs):
    raise NotImplementedError("score_mod_bwd was stripped from this build.")
