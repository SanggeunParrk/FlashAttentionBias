...
# H100-only, minimal happy-path flash attention.
# Adapted from the B200 branch's minimal interface.py — only the arch
# (SM100 -> SM90) and kernel constructor sites differ.
#
# Supported:
# - dtype fp16 / bf16 (working); fp32 (TF32 MMA) raises NotImplementedError
#   inside the kernel's mma() until the TF32 WGMMA path is wired up
# - SM90 only (Hopper H100)
...
_SUPPORTED_ARCHES = (90,)  # H100 only
...
def _flash_attn_fwd(
    ...
    # Fixed tile config for SM90.
    # fp32 doubles per-stage SMEM cost; one K/V stage keeps the tile within
    # H100's 228 KB SMEM budget. fp16/bf16 uses two stages for pipelining.
    if q.dtype == torch.float32:
        ...
        num_stages = 2
    num_threads = 384  # 3 warpgroups (2 MMA + 1 producer) for SM90 fwd
    # Optional override for benchmarking.
    ...
    compile_key = (
        dtype, head_dim, tile_m, tile_n, num_stages,
        lse is None,
    ...
    if compile_key not in _flash_attn_fwd.compile_cache:
        ...
        fa_fwd = FlashAttentionForwardSm90(
            dtype,
            head_dim,
            head_dim,  # head_dim_v == head_dim (MHA)
            1,         # qhead_per_kvhead == 1 (MHA, no GQA)
            is_causal=False,
            is_local=False,
            pack_gqa=False,
            tile_m=tile_m,
